...
create or replace function public.create_pending_order_with_payment_v2(
  p_order_data jsonb,
  p_items jsonb,
...
begin
  ...
  if p_checkout_attempt_id is null then
    raise exception 'PAYMENT_CHECKOUT_ATTEMPT_REQUIRED';
  end if;
  ...
  if not found then
    raise exception 'PAYMENT_CHECKOUT_ATTEMPT_INVALID';
  end if;
  ...
  if not found then
    raise exception 'PAYMENT_CHECKOUT_ATTEMPT_STATE_INVALID';
  end if;
...
revoke execute on function public.create_pending_order_with_payment_v2(jsonb, jsonb, text, text, uuid)
  from public, anon, authenticated;
grant execute on function public.create_pending_order_with_payment_v2(jsonb, jsonb, text, text, uuid)
  to service_role;
