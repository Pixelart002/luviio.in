-- Preserve compare/MRP as part of the immutable invoice-line rendering snapshot.
-- Existing invoice_items remain immutable; legacy PDFs are enriched at read time
-- from the order-item snapshot when this metadata is absent.
...
declare
  ...
  v_invoice_id uuid;
  v_seller_name text; v_seller_addr1 text; v_seller_addr2 text; v_seller_city text;
  v_seller_district text; v_seller_state text; v_seller_state_code text; v_seller_pincode text;
  v_seller_country text; v_seller_pan text; v_seller_gstin text; v_seller_email text; v_seller_website text;
  v_logo_url text; v_signature_url text; v_signatory_name text; v_signatory_designation text;
...
begin
  ...
  if v_gstin_registered and btrim(coalesce(v_seller_gstin,'')) = '' then raise exception 'SELLER_CONFIGURATION_INCOMPLETE: GSTIN is required when seller_gst_registered=true'; end if;
  ...
  values(
    ...
    jsonb_strip_nulls(jsonb_build_object('legal_name',v_seller_name,'address_1',v_seller_addr1,'address_2',nullif(v_seller_addr2,''),'city',v_seller_city,'district',nullif(v_seller_district,''),'state',v_seller_state,'state_code',v_seller_state_code,'pincode',v_seller_pincode,'country',v_seller_country,'pan',nullif(v_seller_pan,''),'gstin',nullif(v_seller_gstin,''),'gst_registered',v_gstin_registered,'email',nullif(v_seller_email,''),'website',nullif(v_seller_website,''),'logo_url',nullif(v_logo_url,''),'signature_url',nullif(v_signature_url,''),'authorised_signatory_name',nullif(v_signatory_name,''),'authorised_signatory_designation',nullif(v_signatory_designation,''))),
    ...
    jsonb_build_object('subtotal',coalesce(v_order.subtotal,0),'discount_amount',coalesce(v_order.discount_amount,0),'shipping_cost',coalesce(v_order.shipping_cost,0),'tax_amount',coalesce(v_order.tax_amount,0),'total_amount',coalesce(v_order.total_amount,0),'currency',coalesce(v_order.currency,'INR'),'tax_type',coalesce(v_order.tax_type,'IGST'))
  ...
  select v_invoice_id,row_number() over(order by oi.id),oi.product_id,coalesce(oi.product_name,p.name,'Product'),p.sku,coalesce(oi.hsn_code,p.hsn_code),oi.quantity,
    round(coalesce(oi.unit_price,0),2),round(coalesce(oi.discount_amount,0),2),
    round(coalesce(oi.subtotal,(oi.unit_price*oi.quantity)-coalesce(oi.discount_amount,0)),2),coalesce(oi.gst_percentage,p.gst_percentage,0),
    ...
    round(coalesce(oi.tax_amount,0),2),round(coalesce(oi.subtotal,(oi.unit_price*oi.quantity)-coalesce(oi.discount_amount,0))+coalesce(oi.tax_amount,0),2),
    jsonb_build_object('source_order_item_id',oi.id,'compare_price',round(coalesce(oi.compare_price,0),2))
