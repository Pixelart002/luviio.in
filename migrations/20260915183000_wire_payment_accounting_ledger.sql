...
CREATE UNIQUE INDEX IF NOT EXISTS payment_ledger_provider_event_uq ON public.payment_ledger(provider,provider_payment_id,entry_type);
CREATE UNIQUE INDEX IF NOT EXISTS payments_provider_payment_id_uq ON public.payments(payment_provider,provider_payment_id) WHERE payment_provider IS NOT NULL AND provider_payment_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS payment_ledger_order_idx ON public.payment_ledger(order_id,created_at DESC);
...
CREATE OR REPLACE FUNCTION public.settle_payment_transaction(p_order_id uuid,p_provider text,p_provider_payment_id text,p_amount numeric,p_user_id uuid,p_payment_method text DEFAULT NULL,p_currency text DEFAULT NULL)
RETURNS text LANGUAGE plpgsql SECURITY DEFINER SET search_path=public,pg_catalog,pg_temp AS $function$
DECLARE v_order public.orders%ROWTYPE; v_existing public.payments%ROWTYPE; v_currency text; v_payment_id uuid; v_attempt integer;
BEGIN
 p_provider:=lower(nullif(trim(p_provider),'')); p_provider_payment_id:=nullif(trim(p_provider_payment_id),''); v_currency:=upper(nullif(trim(p_currency),''));
 ...
 IF FOUND THEN
   UPDATE public.payments SET status='succeeded',amount=p_amount,amount_paise=round(p_amount*100)::bigint,currency=v_currency,payment_method=coalesce(p_payment_method,payment_method),updated_at=now() WHERE id=v_existing.id;
 ...
 ELSE
   ...
   VALUES(p_order_id,p_user_id,CASE WHEN p_provider='stripe' THEN p_provider_payment_id END,p_provider,p_provider_payment_id,p_amount,round(p_amount*100)::bigint,v_currency,'succeeded',p_payment_method,now()) RETURNING id INTO v_payment_id;
 END IF;
