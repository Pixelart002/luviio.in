-- Keep admin payment pagination chronological by actual order creation time.
-- Order numbers are random identifiers and must never be used as a recency key.
...
WITH durable_attempts AS (
  ...
  SELECT r.id,r.order_id,COALESCE(p.amount,o.total_amount,0)::numeric AS amount,
    ...
    o.order_number,o.status AS order_status,o.total_amount,o.created_at AS order_created_at
  FROM public.payment_retry_reservations r
  LEFT JOIN public.payments p ON p.order_id=r.order_id
  LEFT JOIN public.orders o ON o.id=r.order_id
  WHERE r.status='expired'
    AND NOT EXISTS (SELECT 1 FROM public.payment_attempts pa WHERE pa.order_id=r.order_id AND pa.attempt_number=r.attempt_number)
), cod_rows AS (
  SELECT o.id,o.id AS order_id,o.total_amount,ROUND(o.total_amount*100)::bigint,COALESCE(o.currency,'INR')::text,
    ...
    o.created_at,o.updated_at,o.order_number,o.status AS order_status,o.total_amount,o.created_at AS order_created_at
  ...
  UNION ALL SELECT * FROM expired_slots
  UNION ALL SELECT * FROM cod_rows
)
...
FROM events e
ORDER BY e.order_created_at DESC NULLS LAST,e.created_at ASC,e.attempt_number ASC,e.id ASC
