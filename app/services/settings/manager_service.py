...
class ManagerSettingsService:
    ...
    async def update_operational_setting(self, key: str, new_value: Any, manager_id: str) -> Dict[str, Any]:
        existing = await self.engine.fetch_by_key(key)
        
        # Managers can only update "operational" or "ui_ux" categories
        allowed_categories = [SettingCategory.OPERATIONAL.value, SettingCategory.UI_UX.value]
