...
Upgrades:
  1. Converted all Supabase calls to `await`.
  2. Replaced `time.sleep()` with `asyncio.sleep()` to prevent blocking.
"""
import logging
import asyncio
from typing import TYPE_CHECKING
    ...
    *,
    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            ...
            if upd and hasattr(upd, "data") and upd.data:
                logger.info("Stock deducted via lock | sku=%s product=%s qty=-%d %d→%d attempt=%d", sku, product_name, qty, current_stock, new_stock, attempt)
                await _log_audit(sb, product_id, -qty, new_stock, f"order_create_lock:{product_name}", sku=sku)
                return True

            if attempt < max_retries:
                delay = RETRY_DELAY_SEC * (2 ** (attempt - 1))
                logger.warning("Lock conflict (attempt %d/%d) | sku=%s — retrying in %.1fs", attempt, max_retries, sku, delay)
                await asyncio.sleep(delay)  # 🔥 Replaced time.sleep with asyncio.sleep
                continue
            else:
                logger.error("Lock conflict exhausted | sku=%s product=%s", sku, product_name)
                return False
...
async def restore_stock(sb: "AsyncClient", product_id: str, qty: int, context: str = "unknown") -> bool:
    try:
        await sb.rpc("increment_stock", {"p_id": product_id, "p_qty": qty}).execute()
        logger.info("Stock restored via RPC | product=%s qty=+%d ctx=%s", product_id, qty, context)
        await _log_audit(sb, product_id, +qty, None, context)
        return True
    except Exception as rpc_err:
        logger.warning("RPC restore failed — trying UPDATE | ctx=%s: %s", context, rpc_err)

    try:
        row = await sb.table("products").select("stock").eq("id", product_id).limit(1).execute()

        if row and hasattr(row, "data") and row.data:
            current_stock = row.data[0].get("stock", 0)
            new_stock = current_stock + qty
            await sb.table("products").update({"stock": new_stock}).eq("id", product_id).execute()
            logger.info("Stock restored via UPDATE | product=%s qty=+%d %d→%d ctx=%s", product_id, qty, current_stock, new_stock, context)
            await _log_audit(sb, product_id, +qty, new_stock, context)
            return True
        else:
            logger.error("CRITICAL: restore_stock — product not found | product=%s ctx=%s", product_id, context)
            return False
...
async def decrement_stock_batch(sb: "AsyncClient", items: list[tuple[str, int, str]]) -> tuple[list[str], list[str]]:
    succeeded_items = []
    failed_ids = []

    for product_id, qty, name in items:
        if await decrement_stock(sb, product_id, qty, name):
            succeeded_items.append((product_id, qty, name))
        else:
            failed_ids.append(product_id)
            for sid, sqty, sname in succeeded_items:
                await restore_stock(sb, sid, sqty, f"batch_rollback:{sname}")
            return [], [product_id] 

    return [item[0] for item in succeeded_items], failed_ids

async def restore_stock_batch(sb: "AsyncClient", items: list[tuple[str, int, str]]) -> int:
    restored = 0
    for product_id, qty, context in items:
        if await restore_stock(sb, product_id, qty, context):
            restored += 1
    return restored

async def get_stock(sb: "AsyncClient", product_id: str) -> int | None:
    try:
        row = await sb.table("products").select("stock").eq("id", product_id).limit(1).execute()
        if row and hasattr(row, "data") and row.data:
            return row.data[0].get("stock", 0)
...
async def is_in_stock(sb: "AsyncClient", product_id: str, qty: int = 1) -> bool:
    stock = await get_stock(sb, product_id)
    return stock is not None and stock >= qty

async def _log_audit(sb: "AsyncClient", product_id: str, delta: int, stock_after: int | None, reason: str, sku: str | None = None) -> None:
    try:
        row = {"product_id": product_id, "delta": delta, "reason": reason[:200]}
        if stock_after is not None: row["stock_after"] = stock_after
        if sku:
            ...
            try:
                r = await sb.table("products").select("sku").eq("id", product_id).limit(1).execute()
                if r and hasattr(r, "data") and r.data: row["sku"] = r.data[0].get("sku")
            except Exception: pass
        await sb.table("stock_audit").insert(row).execute()
    except Exception as exc:
        logger.debug("Stock audit skipped (table may not exist): %s", exc)