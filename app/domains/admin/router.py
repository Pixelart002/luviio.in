...
async def verify_admin(request: Request, user_id: str = Depends(get_user_id_strict)):
    if hasattr(request.state, "actions"):
        request.state.actions.append(
            f"Initiating live God-Mode verification for: {user_id[:8]}..."
        )
    ...
    if hasattr(request.state, "actions"):
        request.state.actions.append(
            "Access Granted -> Verified as Active System Administrator"
        )
...
async def admin_stats(request: Request, user_id: str = Depends(get_user_id_strict)):
    ...
    if hasattr(request.state, "actions"):
        request.state.actions.append(
            "Successfully aggregated & computed global dashboard metrics"
        )
