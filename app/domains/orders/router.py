...
async def create_order_from_cart(
    ...
    """Initiates checkout through the canonical payment/order flow."""
...
async def get_my_order(
    ...
    is_admin = current_user.get("profile", {}).get("role") in [
...
async def admin_update_order(
    ...
    result = await OrderService().admin_update_order(
        str(order_id), payload.model_dump(exclude_unset=True)
    )
