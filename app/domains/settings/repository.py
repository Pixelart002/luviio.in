...
class AsyncSettingsRepository:
    ...
    async def update_setting_value(self, key: str, new_value: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            updated_data = getattr(update_res, "data", None) or []
            if not updated_data:
                raise RuntimeError("Update matched no settings row.")

            # The installed async PostgREST builder does not support
            # update(...).select(...); fetch the canonical row separately.
            res = await (
                admin_sb.table("system_settings")
                .select("*")
                .eq("key", key)
            ...
            if data:
                return data[0]
            raise RuntimeError("Setting update succeeded but canonical row could not be fetched.")
