...
class AuthService:
    ...
    async def process_reset_password(self, access_token: str, new_password: str) -> None:
        ...
        try:
            ...
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=AuthSecurityMessages.RESET_FAILED.format(reason=str(exc))) from exc
        except RuntimeError as exc:
            raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail=AuthSecurityMessages.RESET_FAILED.format(reason="Service unreachable")) from exc