...
SECURITY & ARCHITECTURE FIXES:
  ...
  8. 🔥 ADDED: Dynamic RBAC / ABAC Guards (require_roles)
  9. 🔥 ADDED: Zero-IDOR Scope Parameterization (get_user_id_strict)
...
# ══════════════════════════════════════════════════════════════════════════════
#  🔥 DYNAMIC RBAC / ABAC GUARDS (ADDED FOR ENTERPRISE SECURITY)
# ══════════════════════════════════════════════════════════════════════════════

def require_roles(allowed_roles: List[str]) -> Callable:
    """
    Role-Based Access Control Factory.
    Validates if the current user possesses one of the explicitly allowed roles.
    
    Usage: @router.post("/path", dependencies=[Depends(require_roles(["admin", "merchant"]))])
    """
    async def role_checker(current_user: dict[str, Any] = Depends(get_current_user)) -> dict[str, Any]:
        profile = current_user.get("profile", {})
        user_role = str(profile.get("role") or "customer")

        if user_role not in allowed_roles:
            logger.warning(
                "RBAC Denied | User: %.8s | Role: %s | Required: %s", 
                profile.get("id", "Unknown"), user_role, allowed_roles
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail=f"Access Denied. Your role '{user_role}' lacks required permissions."
            )
        return current_user
...
require_merchant = require_roles(["admin", "merchant"])


# ══════════════════════════════════════════════════════════════════════════════
#  🔥 ZERO-IDOR PARAMETERIZATION (ADDED FOR STRICT DATA SCOPING)
# ══════════════════════════════════════════════════════════════════════════════
...
async def get_user_id_strict(current_user: dict[str, Any] = Depends(get_current_user)) -> str:
    ...
    Extracts the securely verified `user_id` from the JWT token and profile,
    completely bypassing any user-provided ID in the request parameters.
    
    Usage: 
    async def get_my_orders(user_id: str = Depends(get_user_id_strict)):
        return await repo.fetch_orders(user_id)
    """
    user_id = current_user.get("profile", {}).get("id") or getattr(current_user.get("auth_user"), "id", "")
    if not user_id:
        logger.error("Strict User ID extraction failed: Missing ID in context.")
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, 
            detail="Critical security exception: User identity missing for scope resolution."
        )
    return str(user_id)
