...
async def get_vapid_key(request: Request):
    ...
    return success_response(service.get_vapid_key())
...
async def subscribe(request: Request, payload: PushSubscription, user_id: str = Depends(get_user_id_strict)):
    ...
    result = await service.subscribe(user_id, payload.endpoint, payload.keys.p256dh, payload.keys.auth)
...
async def send_batch_notification(request: Request, payload: BatchNotificationRequest):
    ...
    results = await service.send_batch_notification(payload.user_ids, payload.title, payload.body, payload.icon, payload.url)
