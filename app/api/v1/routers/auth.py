...
from app.api.schemas.auth_dto import RegisterRequest, LoginRequest, ForgotPasswordRequest, ResetPasswordRequest
...
async def register(request: Request, payload: RegisterRequest):
    ...
    await AuthService().register_user(payload.email, payload.password, payload.full_name or "", get_remote_address(request))
...
async def login(request: Request, response: Response, payload: LoginRequest):
    ...
    session_data = await AuthService().login_user(payload.email, payload.password, get_remote_address(request))
...
async def refresh(request: Request, response: Response, refresh_token: str | None = Cookie(None)):
    ...
    response.set_cookie(**_COOKIE_KWARGS, value=session_data["refresh_token"], max_age=7 * 24 * 60 * 60)
    data = {"access_token": session_data["access_token"], "token_type": "bearer", "expires_in": session_data["expires_in"]}
    return success_response(data=data)
...
# ✅ ZERO-TRUST FIX: Token extraction logic
...
async def check_session(request: Request, current: dict[str, Any] = Depends(get_current_user)):
    ...
    data = {"authenticated": True, "user_id": user_id, "email": email, "expires_at": current.get("exp")}
    return success_response(data=data, message=AuthMessages.SESSION_VALID)