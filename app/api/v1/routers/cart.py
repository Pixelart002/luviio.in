...
async def _calculate_cart_pricing(repo: AsyncCartRepository, cart: dict[str, Any]) -> dict[str, Any]:
...
async def get_cart(request: Request, current: dict[str, Any] = Depends(get_current_user)):
...
async def add_item(request: Request, payload: AddItemRequest, current: dict[str, Any] = Depends(get_current_user)):
    user_id = _get_user_id(current)
    ...
    if existing:
        ...
        await repo.add_item_to_cart(cart["id"], product_id, payload.quantity, float(prod["price"]))
...
async def update_item(request: Request, product_id: UUID, payload: UpdateItemRequest, current: dict[str, Any] = Depends(get_current_user)):
    user_id = _get_user_id(current)
    pid = str(product_id)
    repo = AsyncCartRepository()
...
async def remove_item(request: Request, product_id: UUID, current: dict[str, Any] = Depends(get_current_user)):
    user_id = _get_user_id(current)
    pid = str(product_id)
    repo = AsyncCartRepository()
...
async def clear_cart(request: Request, current: dict[str, Any] = Depends(get_current_user)):
    user_id = _get_user_id(current)
    repo = AsyncCartRepository()
...
async def send_cart_reminder(request: Request, cart_id: UUID):
    ...
    push_sent = 0
    try: 
        # Note: We keep the sync Supabase client for integrations as they aren't async yet.
        from app.core.supabase import get_admin_supabase
        sb_sync = get_admin_supabase() 
        push_sent = send_push_to_user(sb_sync, user_id, title="🛒 You left something behind!", body=f"Hi {name or 'there'}, your cart has {len(items)} item(s) waiting.", icon="/icons/ri-shopping-cart-2.png", url="/cart.html")
