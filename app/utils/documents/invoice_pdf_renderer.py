...
def _line_values(item: dict[str, Any], qty: int) -> tuple[float, float, float]:
    """Return displayed unit/MRP, total discount, and actual net selling value."""
    selling_unit = _f(item.get("unit_price"))
    compare_unit = _compare_price(item)
    display_unit = max(compare_unit, selling_unit)
    stored_discount = max(0.0, _f(item.get("discount_amount")))
    derived_discount = max(0.0, (display_unit - selling_unit) * qty)
    discount = stored_discount if stored_discount > 0 else derived_discount
    net = _f(item.get("taxable_value"))
    if net <= 0: net = _f(item.get("subtotal"))
    if net <= 0: net = max(0.0, selling_unit * qty)
    return display_unit, discount, net
...
def build_snapshot_invoice_pdf(invoice_order: dict[str, Any], customer: dict[str, Any], seller_snapshot: dict[str, Any], billing_snapshot: dict[str, Any], shipping_snapshot: dict[str, Any]) -> bytes:
    ...
    widths = [18,148,38,48,22,48,58,66,48,66]
    rows = [[Paragraph("Sl.",ST["head"]),Paragraph("Description",ST["head_l"]),Paragraph("HSN",ST["head"]),Paragraph("Unit Price",ST["head_r"]),Paragraph("Qty",ST["head"]),Paragraph("Discount",ST["head_r"]),Paragraph("Net Amount",ST["head_r"]),Paragraph("GST / Tax",ST["head"]),Paragraph("Tax Amt",ST["head_r"]),Paragraph("Total",ST["head_r"])]]
    items = order.get("order_items") or []
    run_tax = 0.0
    run_net = 0.0
    for idx, item in enumerate(items,1):
        qty = max(1, int(_f(item.get("quantity"),1)))
        display_unit, discount, net = _line_values(item, qty)
        rate = _f(item.get("gst_percentage"))
        item_tax = _f(item.get("tax_amount"))
        if item_tax < 0: item_tax = 0.0
        ...
        tax_display = f"{rate:g}%<br/>{tax_type}"
        name = _s(item.get("product_name"), "Product")
        hsn = _s(item.get("hsn_code"))
        rows.append([Paragraph(str(idx),ST["cell_c"]),Paragraph(name,ST["cell"]),Paragraph(hsn,ST["cell_c"]),Paragraph(_money(display_unit),ST["cell_r"]),Paragraph(str(qty),ST["cell_c"]),Paragraph(_money(discount),ST["cell_r"]),Paragraph(_money(net),ST["cell_r"]),Paragraph(tax_display,ST["cell_c"]),Paragraph(_money(item_tax),ST["cell_r"]),Paragraph(_money(line_total),ST["cell_r"])])
        run_tax += item_tax
        run_net += net

    shipping_cost = _f(order.get("shipping_cost"))
    if shipping_cost > 0:
        rows.append([Paragraph("",ST["cell"]),Paragraph("Shipping Charges",ST["cell_b"]),Paragraph("9965",ST["cell_c"]),Paragraph(_money(shipping_cost),ST["cell_r"]),Paragraph("1",ST["cell_c"]),Paragraph(_money(0),ST["cell_r"]),Paragraph(_money(shipping_cost),ST["cell_r"]),Paragraph("—",ST["cell_c"]),Paragraph(_money(0),ST["cell_r"]),Paragraph(_money(shipping_cost),ST["cell_r"])])
