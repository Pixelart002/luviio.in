...
class AsyncProductRepository:
    ...
    async def get_active_categories(self) -> list[dict[str, Any]]:
        res = await self.admin_sb.table("categories").select("*").eq("is_active", True).execute()
        return getattr(res, "data", None) or []

    async def create_category(self, data: dict) -> dict[str, Any] | None:
        res = await self.admin_sb.table("categories").insert(data).execute()
        return res.data[0] if getattr(res, "data", None) else None

    async def check_active_products_in_category(self, category_id: str) -> int:
        res = await self.admin_sb.table("products").select("id", count="exact").eq("category_id", category_id).eq("is_active", True).limit(1).execute()
        return res.count or 0

    async def soft_delete_category(self, category_id: str) -> bool:
        res = await self.admin_sb.table("categories").update({"is_active": False}).eq("id", category_id).execute()
    ...
    async def get_products(self, page: int, page_size: int, category_slug: str | None, search: str | None, min_price: float | None, max_price: float | None, in_stock: bool | None) -> tuple[list, int]:
        ...
        if category_slug:
            cat = await self.admin_sb.table("categories").select("id").eq("slug", category_slug).limit(1).execute()
    ...
    async def get_product_by_slug(self, slug: str) -> dict[str, Any] | None:
        res = await self.admin_sb.table("products").select("*, categories(name, slug)").eq("slug", slug).eq("is_active", True).limit(1).execute()
        return res.data[0] if getattr(res, "data", None) else None
        
    async def get_product_by_id(self, product_id: str) -> dict[str, Any] | None:
        res = await self.admin_sb.table("products").select("id, images, is_active").eq("id", product_id).limit(1).execute()
        return res.data[0] if getattr(res, "data", None) else None

    async def check_sku_exists(self, sku: str) -> bool:
        res = await self.admin_sb.table("products").select("id").eq("sku", sku).limit(1).execute()
        return bool(getattr(res, "data", None))

    async def generate_unique_slug(self, base_slug: str) -> str:
        slug, counter = base_slug, 2
        while True:
            existing = await self.admin_sb.table("products").select("id").eq("slug", slug).limit(1).execute()
            if not getattr(existing, "data", None): return slug
            slug = f"{base_slug}-{counter}"
            counter += 1

    async def create_product(self, data: dict) -> dict[str, Any] | None:
        res = await self.admin_sb.table("products").insert(data).execute()
        return res.data[0] if getattr(res, "data", None) else None

    async def update_product(self, product_id: str, data: dict) -> dict[str, Any] | None:
        res = await self.admin_sb.table("products").update(data).eq("id", product_id).execute()
        return res.data[0] if getattr(res, "data", None) else None

    async def soft_delete_product(self, product_id: str) -> bool:
        res = await self.admin_sb.table("products").update({"is_active": False}).eq("id", product_id).execute()
        return bool(getattr(res, "data", None))