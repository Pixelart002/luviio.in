...
def upsert_profile(
    ...
    Returns the latest profile row.
    """
    try:
        ...
        logger.error(
            "Failed to upsert user profile | id=%s: %s",
            user_id[:8],
            e,
        )
        raise
...
def get_profile(self, user_id: str) -> dict[str, Any] | None:
    """
    Compatibility alias used by dependencies.py
    """
    return self.get_user_by_id(user_id)
...
def is_address_in_active_order(
    self,
    address_id: str,
) -> bool:
    res = (
