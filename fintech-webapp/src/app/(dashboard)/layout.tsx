...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      {/* Sidebar (Mobile pe chota, Desktop pe bada) */}
      <aside className="w-20 md:w-64 bg-white border-r border-gray-200 flex flex-col transition-all">
        ...
        <nav className="flex-1 py-6 flex flex-col gap-2 px-3">
          ...
          <Link href="#" className="flex items-center gap-3 px-3 py-3.5 text-gray-500 hover:bg-gray-50 hover:text-gray-900 rounded-xl transition">
            <i className="ri-exchange-dollar-line text-xl"></i>
            <span className="hidden md:block font-medium">Transfers</span>
          </Link>
          <Link href="#" className="flex items-center gap-3 px-3 py-3.5 text-gray-500 hover:bg-gray-50 hover:text-gray-900 rounded-xl transition">
